...
**As of Phase 4**, `engine::run_record::{SingleRunRecord, BatchRunRecord}` are exactly this shape minus `id`/`kind`/`created_at` (the server assigns those on ingest — see [api.md](./api.md#post-runs)). `monopoly run --out`/`monopoly batch --out` write these same types directly, so the CLI's local JSON and the archived shape use identical field names (`per_game_summary`/`aggregate_stats`, not the `per_game`/`aggregate` Phase 3 originally used) with no translation layer. A single run's `--out`/archived JSON deliberately omits `final_state` (the live board/player state `monopoly run`'s terminal summary prints): it's fully derivable by replaying `events`, the same determinism-as-storage-optimization principle already applied to batches.

The SQLite table the server stores this in is an implementation detail with no documented schema-stability guarantee — see [api.md](./api.md#implementation-notes-phase-4).
